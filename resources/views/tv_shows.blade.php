...
<section id="projects" class="pt-28 py-20 bg-slate-900">
    <div class="container mx-auto px-4 md:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach ($tv_shows as $card)
<a href="{{ $card->article_url }}" target="_blank" class="block group h-full">
    <div class="project-card group h-full" data-category="{{ $card->category }}">
        <div class="flex flex-col justify-between h-full relative overflow-hidden rounded-xl bg-slate-800 border border-slate-700 transform transition-all duration-500 hover:-translate-y-2">
            ...
            <div class="overflow-hidden relative">
                <img src="{{ asset($card->image_url) }}" alt="{{ $card->title }}"
            ...
            <div class="p-6">
                ...
                <p class="text-gray-400 mb-4">{{ $card->source }}<br>{{ \Carbon\Carbon::parse($card->publication_date)->format('d M, Y') }}</p>
            </div>
